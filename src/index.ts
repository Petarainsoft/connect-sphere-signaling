  ...
  public static run(argv: string[]): RenderStreaming {
    ...
    const readOptions = (): Options => {
      if (Array.isArray(argv)) {
        program
          .usage('[options] <apps...>')
          .option('-p, --port <n>', 'Port to start the server on.', process.env.PORT || `3456`)
          ...
          .option('-m, --mode <type>', 'Choose Communication mode public or private.', process.env.MODE || 'private')
